...
void printRecords(struct Record *records, size_t count) {
    for (size_t i = 0; i < count; ++i) {
        printf("%d-%02d-%02d %02d:%02d:%02d  Status: %d  Code: %d\n", records[i].year, records[i].month,
               records[i].day, records[i].hour, records[i].minute, records[i].second, records[i].status,
               records[i].code);
    }
}
...
void addAndSortRecord(struct Record *records, size_t *count) {
    ...
    // Добавление записи
    printf(
        "Enter year, month, day, hour, minute, second, status, code "
        "(space-separated): ");
    scanf("%d %d %d %d %d %d %d %d", &records[*count - 1].year, &records[*count - 1].month,
          &records[*count - 1].day, &records[*count - 1].hour, &records[*count - 1].minute,
          &records[*count - 1].second, &records[*count - 1].status, &records[*count - 1].code);
